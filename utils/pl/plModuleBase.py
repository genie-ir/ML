...
class plModuleBase(pl.LightningModule):
    ...
    def predefined_net_step_master0(self, net):
        ...
        def predefined_net_step(batch):
            cowsay.cow('NotImplementedError:\nplease define `{}` in `{}:pipline`.'.format(pipline_name, self.__class__.__name__))
            sys.exit()
    ...
    def training_step(self, batch, batch_idx, split='train'):
        ...
        for optimizer_idx in self.optimizationIndex:
            ...
            loss, _ld = getattr(self, '{}_step'.format(cnet))(batch, split='train', batch_idx=batch_idx)
    ...
    def validation_step(self, batch, batch_idx, split='val'):
        ...
        for cnet in self.netconfig:
            loss, _ld = getattr(self, '{}_step'.format(cnet))(batch, split='val', batch_idx=batch_idx)
