...
class VQModel(pl.LightningModule):
    ...
    def netConditins(self, simg, GrayMask):
        Qorg, QR_lesions, QR_surface = self.net(simg, GrayMask)

        y = self.decoder(Qorg)
        yl = self.decoder(QR_lesions)
        ys = self.decoder(QR_surface)
        
        # signal_save(torch.cat([
        #     (simg+1) * 127.5,
        #     torch.cat([GrayMask,GrayMask,GrayMask], dim=1) * 25,
        #     (y+1) * 127.5,
        #     (yl+1) * 127.5,
        #     (ys+1) * 127.5,
    ...
    def net(self, x, GrayMask):
        ...
        Qorg = h_new
        Qrog_detached = Qorg.detach()
        QGfazzy = self.encoder.QGfazzy(Qrog_detached, GrayMask)

        QR_lesions = Qrog_detached * QGfazzy       
        QR_surface = Qrog_detached * (1 - QGfazzy)      

        return Qorg, QR_lesions, QR_surface
    ...
    def netA(self, simg, GrayMask):
        Qorg_NotUsableHere, QR_lesions, QR_surface = self.net(simg, GrayMask)

        # y = self.decoder(Qorg)
        
        # signal_save(torch.cat([
        #     (simg+1) * 127.5,
        #     torch.cat([GrayMask,GrayMask,GrayMask], dim=1) * 255,
        #     (y+1) * 127.5,
    ...
    def pipline(self, xs, Xc, xsf,
        ...
        if optidx == 0 and condstep == True:
            Cond_loss_logdict = {}
            xsng  = xs * C_xsmask
            xsng_surface = xsf * self.netConditins(xsng, False) # only surface and zero diagonals :)
            xsng_GA = xsf * self.netConditins(xsng, True) # GA means all === Global Attributes :)
            Cond_loss_surface, Cond_loss_logdict_surface = self.loss.geometry(xsng, xsng_surface, split=split + 'Cond_Geo_surface', losscontroller='CondGeo')
            Cond_loss_grayArea, Cond_loss_logdict_grayArea = self.loss.geometry(xs, xsng_GA, split=split + 'Cond_Geo_GA', pw=0)
            Cond_loss = Cond_loss_surface + Cond_loss_grayArea
            Cond_loss_logdict = {**Cond_loss_logdict_surface,**Cond_loss_logdict_grayArea}
            self.report('Cond_surface', 2, [xsng, xsng_surface])
            self.report('Cond_GA', 2, [xsng, xsng_GA])
